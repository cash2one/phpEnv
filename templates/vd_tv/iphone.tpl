					...
					<select class="wa-vdtv-selector">
						...
						{%for $i=0 to $me%}
							...
							<option value="{%$i*10%}" {%if $i==0 %} selected=1 {%/if%}>
								{%if $me== ($r1.vlink_num_baidu - 1)/10%}
									{%$text="第{%$r1.vlink_num_baidu%}集"%}
								{%else%}
									{%$text="第{%$to%}-{%$from%}集"%}
								{%/if%}
        ...
        {%if $vcount>0%}
            <div class="wa-vdtv-vlist  c-gap-top-large">
                    ...
                    {%foreach $list as $index=>$item%}
                        ...
                        {%fe_fn_c_box_adaptive_prefix  url=$item.linkurl ltj="l" class="wa-wz-tvseries-tab-list-cell wa-wz-tvseries-wa-video-openbox"%}
                            {%$item.linkcontent%}
                            {%if $item.latest %}
                                <i class="c-text-box c-text-box-red" style="position:absolute;top:0px; right:0px;">新</i>
                            {%/if%}
                        {%fe_fn_c_box_adaptive_suffix  url=$item.linkurl%}
                        {%if $index % 5 == 4%}
                            </div>
                         {%/if%}
...
A.setup({
	...
	vlink_api:'{%"http://opendata.baidu.com/api.php?format=json&ie=utf-8&oe=utf-8&resource_id=6869&from_mid=1&co=vlink&arn=10&need_di=1"|getHttpsHost%}' + '&query='+ '{%$tplData.fetchkey%}' +'&locsign={%$r1.se_locsign%}',
